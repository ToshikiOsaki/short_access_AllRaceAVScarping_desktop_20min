...
<img width="1917" height="1013" alt="平均タイム取得アプリ画面アクセス短縮版" src="https://github.com/user-attachments/assets/ee9e92e9-6c03-4fa0-a578-48c5e1c57713" />
（デスクトップアプリの起動画面）
...
- 平均の集計に使うデータ区間が5年分でそれを１年ずつズラしながら６セットまとめて取得する時のプログラム作動時間の目安はだいたい20分です。（平たく言って６年分の全レース条件の平均を取得したいときは大体20分くらいかかると思ってください）
- プログラムを起動する前にExcelを開いてExcelを自分で作成して、その後ファイルのパスをコピーしてアプリの入力欄に張り付けてください。張り付ける時には両端の"を取らなくてもプログラム側で勝手に削ってくれる仕様にしているので問題なく作動します。（自分でExcelファイルを作らないとデータが保存されないので注意です！）
- 馬場状態を一つもチェックしなかった場合は、全ての馬場状態のデータを取得します。Excelに記録する時のレース条件の欄がスッキリするのであえて全部チェックするのではなく、全てのチェックを外すのもオススメです。
- プログラムの動作を早くするために、一度別のスクレイピングプログラム（非掲載）でここ6年で一番競争条件の多かった2023年の全レース条件を取得し、それをリスト化してフィルタリングを行うことで存在しないURLへのアクセスを防ぎ処理を高速化しています。なので厳密に言うと年ごとに開催がまちまちな超マイナーなレース条件に関しては取得していないかもしれません。（ただそういうレース条件は基本的にサンプル数が少なすぎて平均としての価値がないので問題はないと考えています）
- 集計期間を１年にして近年で平均を取った時、例えば年によっては京都競馬場の競争のデータがごっそりと存在しないことなどがあると思いますが、これはミスではなく京都競馬場が改修工事に入りそもそも競争が行われていなかった期間のためです。
- 当たり前ですが、開始年や終了年などの入力欄に西暦を手打ちする時に無茶な数字を入れるとプログラムは上手く作動しません。常識の範囲内で正しく使ってください。

## 作った上での工夫
- Excelファイルを張り付ける時に『"』の処理が面倒なので『"』を自動でとるプログラムを追加するなど使用感にもこだわっている。
- とにかく処理を早くしたかったのでAIが最初に提示してきたコードの、例えば2020年の全レース条件のリクエストを送ってデータを取得してから2021年～と行くロジックを変更して、2020年と2021年を合算したページのリクエストを送りページの表中の開催日を参照して各年の平均計算を年ごとにまとめて行うロジックに変更したので、リクエストとsleepの回数が減り、フィルタリング抜きでも格段にプログラム作動時間が短縮した。AIの提案を鵜呑みにするのではなく仕様設計やUX,UIの部分ではこだわって作成した。
- 前回アプリを起動した時のチェックボックスの状況が保存される設計にしています。個人的にはその方が使用感がよくなると思ったためです。

（ほかにも色々とあったはずなんですが、思い出し次第追記しようと思います）  
