...
概要
このアプリケーションは、netkeiba.com のデータベースから、指定した様々な条件に合致する競馬のレースデータをスクレイピングし、平均タイムを算出してExcelファイルに出力するデスクトップツールです。
...
必要なライブラリのインストール リポジトリに requirements.txt という名前のファイルを作成し、以下の内容を記述してください。
...
使い方
プログラムの実行 ターミナル（コマンドプロンプト）で以下のコマンドを実行して、アプリケーションを起動します。

bash
python AIが作りなおした短縮版競馬平均算出アプリデスクトップいけんの？成功2.py
条件の設定
